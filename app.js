...
async function initFinancePage() {
  ...
  allRows._renderer = r => {
    ...
    </tr><tr class="expand-row" style="display:none"><td colspan="7">
      <div class="expand-content">
...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    if (!document.getElementById('delay-meter-style')) {
      ...
      dst.textContent=`
        @keyframes delayPulse {
          0%,100%{ filter:drop-shadow(0 0 0px #f43f5e); opacity:1; }
          50%{ filter:drop-shadow(0 0 10px #f43f5e); opacity:.78; }
        }
        .td-meter { text-align:center; vertical-align:middle; padding:4px 6px !important; }
        .inline-meter-wrap { display:inline-flex; flex-direction:column; align-items:center; gap:1px; }
        .inline-meter-wrap.delayed { animation:delayPulse 1.3s ease-in-out infinite; }
      `;
      document.head.appendChild(dst);
    }

    /* ── Helper: build a compact inline half-circle SVG meter ── */
    function buildInlineMeter(delayVal, rowIdx, suffix) {
      const hasDv=!isNaN(delayVal);
      if (!hasDv) return '<span style="color:rgba(255,255,255,.25);font-size:.7rem">—</span>';
      const isDelayed=delayVal>0, isAhead=delayVal<0;
      const col=isDelayed?C.red:isAhead?C.green:C.muted;
      const fillPct=Math.min(100,(Math.abs(delayVal)/maxDelayAbs)*100);
      const cx=40,cy=38,rad=30;
      const bgArc=`M ${cx-rad} ${cy} A ${rad} ${rad} 0 0 1 ${cx+rad} ${cy}`;
      let fillArc='';
      if (fillPct>0) {
        const angle=Math.PI-(fillPct/100)*Math.PI;
        const ex=cx+rad*Math.cos(angle), ey=cy-rad*Math.sin(angle);
        fillArc=`M ${cx-rad} ${cy} A ${rad} ${rad} 0 ${fillPct>50?1:0} 1 ${ex.toFixed(2)} ${ey.toFixed(2)}`;
      }
      const needleAngle=Math.PI-(fillPct/100)*Math.PI;
      const needleLen=rad-4;
      const ntx=cx+needleLen*Math.cos(needleAngle), nty=cy-needleLen*Math.sin(needleAngle);
      const bx=cx+(needleLen-5)*Math.cos(needleAngle), by=cy-(needleLen-5)*Math.sin(needleAngle);
      const px=Math.sin(needleAngle)*3.5, py=Math.cos(needleAngle)*3.5;
      const ticks=[25,50,75,100].map(tp=>{
        const ta=Math.PI-(tp/100)*Math.PI;
        const x1=cx+(rad-5)*Math.cos(ta), y1=cy-(rad-5)*Math.sin(ta);
        const x2=cx+(rad+2)*Math.cos(ta), y2=cy-(rad+2)*Math.sin(ta);
        return `<line x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}" stroke="rgba(255,255,255,.15)" stroke-width="1"/>`;
      }).join('');
      const gmId=`gm-inline-${suffix}-${rowIdx}`;
      const statusLbl=isDelayed?`⚠ +${Math.round(delayVal)}d`:isAhead?`✅ ${Math.round(Math.abs(delayVal))}d ahead`:`✔ On time`;
      return `<div class="inline-meter-wrap${isDelayed?' delayed':''}">
        <svg viewBox="0 0 80 42" width="80" height="42" style="overflow:visible">
          <defs>
            ...
            ${isDelayed?`<filter id="dgf-${suffix}-${rowIdx}"><feGaussianBlur stdDeviation="2" result="cb"/><feMerge><feMergeNode in="cb"/><feMergeNode in="SourceGraphic"/></feMerge></filter>`:''}
          </defs>
          <path d="${bgArc}" fill="none" stroke="rgba(255,255,255,0.07)" stroke-width="7" stroke-linecap="round"/>
          ${ticks}
          ${fillArc?`<path d="${fillArc}" fill="none" stroke="url(#${gmId})" stroke-width="7" stroke-linecap="round" ${isDelayed?`filter="url(#dgf-${suffix}-${rowIdx})"`:''}/>`:''}
          <path d="M ${cx} ${cy} L ${bx.toFixed(2)} ${by.toFixed(2)}" stroke="${col}" stroke-width="1.5" stroke-linecap="round" opacity="0.9"/>
          <path d="M ${ntx.toFixed(2)},${nty.toFixed(2)} L ${(bx-px).toFixed(2)},${(by+py).toFixed(2)} L ${(bx+px).toFixed(2)},${(by-py).toFixed(2)} Z" fill="${col}" style="filter:drop-shadow(0 0 ${isDelayed?4:2}px ${col})"/>
          <circle cx="${cx}" cy="${cy}" r="2.5" fill="${col}" stroke="rgba(0,0,0,.5)" stroke-width="1"/>
          <text x="${cx}" y="${cy-8}" text-anchor="middle" font-size="9" font-weight="800" fill="${col}" font-family="monospace">${isDelayed?'+':''}${Math.round(delayVal)}d</text>
        </svg>
        <div style="font-size:.58rem;font-weight:700;color:${col};letter-spacing:.3px;line-height:1">${statusLbl}</div>
      </div>`;
    }
    ...
    rows._renderer=r=>{
      ...
      // On-Going column shows meter only for On-Going rows; Completed column only for Completed rows
      const ongoingCell  = isOng       ? buildInlineMeter(delayVal, rowIdx, 'og') : '<span style="color:rgba(255,255,255,.15);font-size:.7rem">—</span>';
      const completedCell= !isOng      ? buildInlineMeter(delayVal, rowIdx, 'cp') : '<span style="color:rgba(255,255,255,.15);font-size:.7rem">—</span>';
      ...
      return `<tr${isDelayed?' class="row-expired"':''}>
        ...
        <td class="td-meter">${ongoingCell}</td>
        <td class="td-meter">${completedCell}</td>
